...
double GenomaVentana::costoTotal( double costo_vidrio, double costo_aluminio )
{
    ...
    foreach( TemplateVentana templ, _fenotipo ) {
        ...
        costo_total += 2.0 * costo_aluminio * templ.alto() * (double)templ.getCantidad();
        costo_total += 2.0 * costo_aluminio * templ.ancho() * (double)templ.getCantidad();

        costo_total += costo_vidrio * ( templ.ancho() * templ.alto() ) * (double)templ.getCantidad();
    }

...
double FFDW::optimizar( QVector<int> gen )
{
    ...
    _orden_barras.append( _largo_barra_original, 0 );

    // Recorro todos los elementos del gen
    for( int pos_gen = 0; pos_gen < gen.size(); pos_gen++ ) {

        // Busco el largo del elemento a cortar
        double longitud = _templates.at( gen.at( pos_gen ) );

        bool entro_existente = false;

        for( int pos_barras = 0; pos_barras<_barras.size(); pos_barras++ ) {

            if( _barras.at( _orden_barras.at( pos_barras ) ).acepta( longitud ) ) {
                _barras.at( _orden_barras.at( pos_barras ) ).hacerCorte( longitud );
        ...
        if( !entro_existente ) {
            ...
            BarraAluminio barra;
            barra.setearLargo( _largo_barra );
...
void FFDW::regenerarOrden()
{
    ...
    for( int i=0; i<_barras.size(); i++ ) {
        orden.append( _barras.at( i ).largoActual(), i );
    }
    _orden_barras = orden.values();
}
